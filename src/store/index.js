...
export default new Vuex.Store({
	...
	actions: {
		generateData({ commit, state }) {
			let auxTickets = [];
			let origin = state.cities[parseInt(Math.random() * (state.cities.length - 0) + 0)];
			let destiny = state.cities[parseInt(Math.random() * (state.cities.length - 0) + 0)];
			let airline2 = state.airlines[parseInt(Math.random() * (state.airlines.length - 0) + 0)];
